...
if($city_id === '0' && $sub_name === undefined){
	$tutors_sql = "select * from agencynew_info ORDER BY rand() LIMIT 50";
}else if($city_id !== '0' && $sub_name !== undefined){
	$tutors_sql = "select * from agencynew_info WHERE city = {$city_id} && subjects LIKE '%$sub_name%' ORDER BY rand() LIMIT 5";
}
else{
	$tutors_sql = "select * from agencynew_info WHERE city = {$city_id} ORDER BY rand() LIMIT 50";
}
...
// 	$tutors_sql = "select * from agencynew_info WHERE city = {$city_id} ORDER BY rand() LIMIT 15";
