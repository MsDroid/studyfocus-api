...
$tutors_sql = "select * from tutor_info WHERE f_name LIKE '%{$search}%' ORDER BY rand() LIMIT 50";
...
// tutors
// $tutors_sql = "select * from tutor_info WHERE f_name LIKE '%{$search}%' LIMIT 12";
// $tutors_query = mysqli_query($con, $tutors_sql);
// $tutors_data = mysqli_fetch_all($tutors_query, MYSQLI_ASSOC);
// array_push($tutors, $tutors_data);
// array_push($result, array('tutor' => $tutors ));

$tutors_sql = "select * from agencynew_info WHERE name LIKE '%{$search}%'";
$tutors_query = mysqli_query($con, $tutors_sql);
