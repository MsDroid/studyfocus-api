...
$blogs = allBlogs($fields, $table);
// subjects
$subjects_sql = "select * from subjects ORDER BY rand() LIMIT 6";
$sub_query = mysqli_query($con, $subjects_sql);
$sub_data = mysqli_fetch_all($sub_query, MYSQLI_ASSOC);

if($blogs != ''){

	echo json_encode(array('message'=>'success','status'=>201,'data'=>$blogs, 'subjects' => $sub_data));
